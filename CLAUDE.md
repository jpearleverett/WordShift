...
- **Audio**: expo-audio playing a bundled 17-sound WAV SFX pack (`assets/sounds/`, 14 base chimes + 3 `*_dark` Phase-3+ variants; settings-gated, plays in iOS silent mode). Migrated off the deprecated expo-av, whose `VideoViewModule` failed to load on SDK 56 (`NoClassDefFoundError`) and crashed the app at launch
...
- **Sound**: a 17-sound SFX pack (cheerful candy chimes + a low `phase_change` swell + `valid_move/victory/perfect_dark` Phase-3+ variants)
...
When a letter is selected, ghost previews show what word would form at each slot position. Valid words green with a "✓ " prefix, invalid red/dimmed with "✗ " (validity is never conveyed by color alone). A slot shows ✓ only when **both** resulting words are valid — the preview ANDs source-word validity (removal must leave a valid source), so there are no false-positive ✓s. Computed via `useMemo` in `usePuzzleGame.ts`, passed to `Row.tsx` as `slotPreviews`. Drop slots also **announce** the preview to screen readers — the slot's accessibility label includes the formed word + valid/invalid (deliberately literal, not phase copy); when previews are suppressed it falls back to the plain positional label. Previews are suppressed ONLY in **Blind Offering** (suppression also disables the drag valid-slot snapping tell and the double-shift `drop1` look-ahead). Challenge mode KEEPS its previews as of the 2026-07 trial-ladder rebalance — losing them made the mode unreadable rather than harder — and its multipliers were re-priced accordingly (1.25x amber / 1.5x phase progress).
...
One-time events at specific puzzle counts (`MICRO_BEATS` in `phaseNarrative.ts`, keyed by exact count, each fires once). Early/mid beats (5–130) break the Phase 1-2 retention valley; **mid-game valley beats (140/155/170/185)** keep the narrative pulse alive through the ~130–185 stretch (Phase-4 reveal ~155; with the descent trio the house itself now completes at the 190 gate, so the beats bridge into both the reveal and the final rooms). `glitch_title` type briefly shows wrong victory text. `ambient_whisper` type shows atmospheric messages.
...
- **SFX pack**: 17 WAV chimes in `assets/sounds/` (14 base + `valid_move_dark`/`victory_dark`/`perfect_dark` Phase-3+ variants), generated by `scripts/tools/generateSounds.mjs`
...
Engaged players can reach Phase 4 in ~155 puzzles instead of ~250 — the `MIN_PUZZLES_FOR_PHASE` floors bind once weighted progress clears the thresholds ({2: 40, 3: 135, 4: 155, 5: 210} in gameBalance.ts):
