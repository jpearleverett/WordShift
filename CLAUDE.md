...
- **Narrative descent into the hands**: finale **dwell gate** (`FINALE_DWELL_PUZZLES` + `recordPhase4Dwell` — the reveal is played, not flashed past); dread-supply rationing + Phase-4 recycle in `puzzleBank.ts`; **animals request offerings** (`offeringRequests.ts`); fed dread words **remembered by name** at the pit (`getStrongestDreadWord`/`getDreadOfferingLine`); **dark move/victory SFX at Phase 3+** (`*_dark.wav`, `setAudioPhase` mirror); a scripted **silent-victory** micro-beat (v1.3.0: puzzle 148, `isSilentVictoryBeat` suppresses the fanfare); **echo puzzles** now run from Phase 3 (pre-finale), not Phase 5 only.
...
| Bamboo Attic (Bamboo the Red Panda) | 112 |
| Star Loft (Vesper the Tarsier) | 126 |
| Belfry (Tock the Aye-aye) | 140 |
| Sky Garden (Moss the Kakapo) | 152 |
