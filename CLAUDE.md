...
- The full suite has ~1,860 tests across 79 suites, expected green (counts drift as features land — don't treat the number as load-bearing). **Prefer running only the relevant test file(s)** rather than the full suite unless explicitly asked to run everything.
...
- **Daily + retention**: daily **week-ramp** (`getDailyRamp`, Mon→Sun) + **streak decay-to-milestone** + **narrative host** (`getDailyHost`/`getDailyHostName`, only ever names a met animal) + a **leaderboard re-check** (tap the completed daily card); **win-back ladder** extended to +1/+3/+7/+14/+30; **interstitials** suppressed at Phase 4 and halved at Phase 3; a **store-review policy** (`reviewPrompt.ts`) that fires only on a Phase 0-1 perfect win and is hard-suppressed from Phase 2 (review-bomb guard).
...
- **Pixel typography** (`theme/fonts.ts`): the cottage chrome renders text in **Pixelify Sans** (OFL, a *legible* pixel font — the previous system font read as "a big blob of normal text in a pixely box"). `PIXEL_FONT` (`PixelifySans-Regular`) / `PIXEL_FONT_BOLD` (`PixelifySans-Bold`); TTFs in `assets/fonts/`, embedded natively via the `expo-font` config plugin in `app.json`, and registered at boot by `loadPixelFonts()` (awaited in App's bootstrap `Promise.all`, so the font is warm before first render — no flash of system font; failure falls back to the system font). Applied to cottage bevel/plaque labels (`CandyButton`, `PixelPlaque`), the FoxGuide bubble, and the HomeScreen dialogue body/name/choice buttons. Long informational body copy (Store/Stats prose) stays on the system font for readability; revert via the two constants if ever unwanted. Jest stubs `expo-font` via `moduleNameMapper` (`__mocks__/expoFont.ts`) since the real package ships an untransformed ESM build.
