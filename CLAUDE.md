...
- **World art (wired into HouseWorld)**: `roof.png` (792×283, chimney + attic window baked in; smoke puffs animate above the chimney at ~25% from the left), `foundation.png` (792×120, stone courses with **grass tufts + a center dirt path gap baked into the base** so the house plants itself into the meadow), `wall.png` (seamless 128×128 timber tile repeated behind the rooms — replaced the flat wall color that read as a sticker), and the tappable `pit_entrance.png` (460×540, stone-rimmed glowing well + a stone path that leads up under the foundation via `PIT_MARGIN_TOP`, aligned to the foundation's dirt gap) are **AI-generated detailed pixel art matching the room interiors** — sources live in `assets/raw/` and are processed (background key → crop → downscale; the pit also drops the top 45% of its tapered path tail) by `scripts/tools/processRawWorldArt.mjs`; their aspect constants live in HouseWorld.tsx and the seat math in `skyGeometry.test.ts` (update both if raws change). **Phase lighting**: `PHASE_HOUSE_TINT` washes the house into each sky's ambient light — a same-source `tintColor` overlay Image follows the roof/foundation/pit silhouettes, and two body scrims tint the wall/frame at full exterior strength while tinting the rooms at ~half (so lit interiors keep glowing against the darkened shell); the pit's tint is capped so the well's teal glow survives the night phases. **Grounding**: the grass is part of the foundation art, and `house_shadow.png` (a soft contact band, `tintColor`+opacity from `CONTACT_SHADOW`) darkens the base — both phase-aware (green meadow → sunset earth → night blues, softening as the sun leaves). `house_shadow.png` is generated by `processRawWorldArt.mjs` (procedural, no raw source). `cloud_1/2.png` remain procedural (`scripts/tools/generatePixelWorld.mjs`, now clouds/tree/ground only). `ground.png`/`tree.png` exist but are intentionally NOT rendered: the sky backgrounds are full landscapes with their own grass and trees. The soft `shadow_figure.png` (the entity) is deliberately NOT pixel art — it should read as otherworldly against the pixel world (`generateWorldArt.mjs`)
