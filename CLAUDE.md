...
- **Dialogue flickering, cloud flicker, and pit particle flicker fix (2026-03-04)**:
  ...
  - **CloudLayer extraction**: Cloud emoji views were rendered inline in HouseWorld's large render tree, so any HouseWorld re-render (room/animal/ritualWords prop changes) caused cloud style recalculation and visual flicker. Extracted into isolated `CloudLayer` React.memo component (same pattern as existing `ParticleLayer`). Cloud animations, styles, and rendering fully self-contained — HouseWorld re-renders no longer affect clouds.
