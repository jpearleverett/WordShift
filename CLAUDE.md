...
1. `isProcessingVictory` lock prevents double-tap
...
- **World art (generated, wired into HouseWorld)**: `shadow_figure.png` (the entity), `cloud_1/2.png` (drifting Phase 0-2, static under reduced motion), `ground.png` (grassy hill under the house, dimming at Phase 3+), `tree.png` (flanking trees that fade as phases darken). Roof/foundation remain the original styled views (chimney smoke, attic window). Generated by `scripts/tools/generateWorldArt.mjs`
- **UI sprites**: `assets/ui/star_filled/star_empty/amber.png` replace the ⭐/💎 emoji for stars and amber in VictoryModal (cross-platform consistent); other emoji (🔥 🏆 📋) are intentional accents with accessibility labels
...
Edit constants in `wordHistory.ts`: `HARD_COOLDOWN` (15), `SOFT_COOLDOWN` (40), `MAX_HISTORY_SIZE` (100)
