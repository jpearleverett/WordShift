...
- **Pre-generated puzzle bank system**:
  - **HARD and MEDIUM_PLUS puzzles served from banks** of 500 pre-generated, pre-validated puzzles instead of always generating on-device in real-time.
  - **Four banks**: `puzzleBankHard.ts` (500 standard HARD), `puzzleBankReverseHard.ts` (500 reverse HARD), `puzzleBankReverseMediumPlus.ts` (500 reverse MEDIUM_PLUS). Auto-generated by scripts in `scripts/`.
  ...
  - **Recycling**: Tracks played puzzle IDs in AsyncStorage. When all exhausted, recycles the oldest-played half. Standard, reverse, and MEDIUM_PLUS reverse banks have independent played-ID lists.
  - **Graceful fallback**: On bank selection failure, silently falls back to on-device real-time generation.
  - **Variants covered**: `standard`, `reverse`, and `reverse_blind` HARD puzzles use the bank; `reverse` and `reverse_blind` MEDIUM_PLUS puzzles use the MEDIUM_PLUS bank; all other difficulty/variant combos still generate on-device.
...
- `startNewGame(difficulty?, mode?, variant?)` - Start a new puzzle: serves curated early puzzles → tries pre-generated bank (HARD standard/reverse/reverse_blind, MEDIUM_PLUS reverse/reverse_blind) → falls back to on-device generation. Uses the selected/preferred variant (or explicit override)
...
Serves pre-validated puzzles for HARD and MEDIUM_PLUS difficulties instead of always generating on-device:

- **Banks**: 500 standard HARD puzzles (`puzzleBankHard.ts`) + 500 reverse-solvable HARD puzzles (`puzzleBankReverseHard.ts`) + 500 reverse-solvable MEDIUM_PLUS puzzles (`puzzleBankReverseMediumPlus.ts`). Auto-generated by scripts in `scripts/`.
- **`PreGeneratedPuzzle` interface**: `id`, `words`, `solution`, `reverseSolution?`, `wordLength`, `qualityScore`, `dreadTier` (0-4), `dreadWordCount`, `allWords`, `semanticTags`
...
- **Recycling**: Tracks played puzzle IDs per bank via AsyncStorage (standard, reverse HARD, and reverse MEDIUM_PLUS tracked independently). When all exhausted, recycles the oldest-played half.
- **Variant routing**: Standard HARD → `PUZZLE_BANK_HARD`. Reverse/reverse_blind HARD → `PUZZLE_BANK_REVERSE_HARD`. Reverse/reverse_blind MEDIUM_PLUS → `PUZZLE_BANK_REVERSE_MEDIUM_PLUS`. All other combos → real-time generation.
- **Integration**: `usePuzzleGame.ts` `startNewGame()` checks the bank first for eligible difficulty/variant combos. On failure, silently falls back to on-device generation.
...
- HARD standard/reverse/reverse_blind and MEDIUM_PLUS reverse/reverse_blind puzzles served from pre-generated banks (500 each); other difficulty/variant combos generate on-device
