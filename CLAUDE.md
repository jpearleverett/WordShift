...
### Challenge Mode
The middle rung of the trial ladder: difficulty-aware undo limit (EASY/MEDIUM=2, MEDIUM_PLUS/HARD=1), no hints; previews follow the global validity-gating rule (graded on EASY only). 1.25x amber; counts 1.5x toward phase progression. **Blind Offering is the apex rung** (toggle gated at 100 puzzles; mutually exclusive with Challenge in the setup menu, runs under `gameMode 'challenge'` internally; four preset combos in `COMBO_PRESETS` compose it with variants): all Challenge limits PLUS previews hidden and free moves judged once at the end of the chain — 2x amber, 2x phase progress (the pacing cap), and a mercy: once the end-of-board judgment has failed, undos stop charging against the limited budget for the rest of that board (`blindFailFreeUndosRef` — a fail must always be repairable). The VictoryModal labels the trial bonus line 'Blind Offering' vs 'Challenge' via `VictoryData.blind`.
