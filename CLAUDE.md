...
- The full suite has ~1,674 tests across 70 suites, expected green (counts drift as features land — don't treat the number as load-bearing). **Prefer running only the relevant test file(s)** rather than the full suite unless explicitly asked to run everything.
...
├── src/__tests__/               # ~1,674 tests, 70 suites
