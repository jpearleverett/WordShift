...
│       ├── gameAlert.ts         # In-game replacement for Alert.alert: showGameAlert(title, message?, buttons?) routes to the cottage-skinned components/ui/GameAlertModal host (mounted once at the App root, one alert at a time + queue) so popups match the skin + single-typeface rule
...
- **Typography** (`theme/fonts.ts`): a **single-typeface system — Shantell Sans everywhere** (SIL OFL; every screen, every element, intentionally no second face). Four faces are registered by ROLE (`SHANTELL_REGULAR/BOLD/ITALIC/BOLD_ITALIC`); bold text uses the BOLD family and italic the ITALIC family — the family name carries the weight/slant, so a `fontWeight`/`fontStyle` on top always resolves to a real face and iOS never falls back to the system font (the trap the single-weight Kurale hit). The legacy-named constants (`PIXEL_FONT`/`PIXEL_FONT_BOLD`/`BODY_FONT`/`BODY_FONT_BOLD`/`BODY_FONT_ITALIC`) are aliases onto the Shantell faces so call sites didn't churn. Registration is **platform-split**: ANDROID embeds the fonts natively via the `expo-font` config plugin (`app.json` → plugins → expo-font → android.fonts builds a real multi-weight `<font-family>` XML per face — runtime `Font.loadAsync` on Android only registers the NORMAL style, so bold would silently fall back to the system font; hence NO loadAsync on Android); iOS loads at runtime via `loadPixelFonts()` (awaited in App's bootstrap `Promise.all`, warm before first render; failure falls back to the system font). Jest stubs `expo-font` via `moduleNameMapper` (`__mocks__/expoFont.ts`) since the real package ships an untransformed ESM build.
