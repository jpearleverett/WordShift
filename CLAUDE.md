...
- **Pre-generated puzzle bank system**:
  - **HARD puzzles now served from a bank** of 500 pre-generated, pre-validated puzzles instead of always generating on-device in real-time.
  - **Two banks**: `puzzleBankHard.ts` (500 standard HARD puzzles) and `puzzleBankReverseHard.ts` (500 reverse-solvable HARD puzzles). Auto-generated by scripts in `scripts/`.
  ...
  - **Recycling**: Tracks played puzzle IDs in AsyncStorage. When all exhausted, recycles the oldest-played half. Standard and reverse banks have independent played-ID lists.
  - **Graceful fallback**: On bank selection failure, silently falls back to on-device real-time generation.
  - **Variants covered**: `standard`, `reverse`, and `reverse_blind` HARD puzzles use the bank; all other difficulty/variant combos still generate on-device.
...
│   ├── data/
│   │   ├── puzzleBankTypes.ts   # PreGeneratedPuzzle interface (id, words, solution, dreadTier, etc.)
...
- `startNewGame(difficulty?, mode?, variant?)` - Start a new puzzle: serves curated early puzzles → tries pre-generated bank (HARD standard/reverse/reverse_blind) → falls back to on-device generation. Uses the selected/preferred variant (or explicit override)
...
- **Phase-tiered dread words**: 336 words split into 4 tier sets (curiosity → emptiness → dread → cosmic). Scoring weights by tier proximity to current phase (same tier: 1.0×, adjacent: 0.5×, distant: 0.15×)
...
Serves pre-validated puzzles for HARD difficulty instead of always generating on-device:

- **Banks**: 500 standard HARD puzzles (`puzzleBankHard.ts`) + 500 reverse-solvable HARD puzzles (`puzzleBankReverseHard.ts`). Auto-generated by scripts in `scripts/`.
- **`PreGeneratedPuzzle` interface**: `id`, `words`, `solution`, `wordLength`, `qualityScore`, `dreadTier` (0-4), `dreadWordCount`, `allWords`, `semanticTags`
...
- **Recycling**: Tracks played puzzle IDs per bank via AsyncStorage (standard and reverse tracked independently). When all exhausted, recycles the oldest-played half.
- **Variant routing**: Standard HARD → `PUZZLE_BANK_HARD`. Reverse/reverse_blind HARD → `PUZZLE_BANK_REVERSE_HARD`. All other combos → real-time generation.
...
- HARD standard/reverse/reverse_blind puzzles served from pre-generated bank (500 each); other difficulty/variant combos generate on-device
