...
- **NEVER** apply `borderRadius`/RN borders over the PNG frames (CSS-rounding baked pixel corners is the #1 cozy-pixel anti-pattern), and content padding must clear the frame: panel edge = 24dp (`PANEL_EDGE_DP`), card edge = 15dp (`CARD_EDGE_DP`).
- Skinned surfaces: PanelCard + CandyButton internally (everything using them inherits), HomeScreen's HubRow/BevelRowButton + all its modal containers (sheets use `openBottom`), StoreModal/PatronModal cards, AchievementToast, Settings/Stats section plaques. Button ink colors ship in the registry (`skin.ink`), contrast-audited ≥4.5:1 per skin.
