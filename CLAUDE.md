...
**Conflict guard**: `uploadToCloud(force = false)` skips the write and records `conflictDetected` in the sync status when the cloud row is **newer than this device's save** (another device played since), so a stale device can never silently clobber real progress. Settings → **Backup & Restore** surfaces the conflict with an explicit choice — restore the newer cloud save, or keep this device's (force-upload); a successful download/restore clears the flag. `performNewCycle` (SettingsScreen) force-uploads so the new cycle always becomes the cloud row.
