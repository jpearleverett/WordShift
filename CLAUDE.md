...
- The full suite has ~85 suites (thousands of tests), expected green (counts drift as features land — don't treat the numbers as load-bearing). **Prefer running only the relevant test file(s)** rather than the full suite unless explicitly asked to run everything.
...
A second round driven by a follow-up external review. All verified and shipped: **hygiene purge #2** (anatomical/proper-noun/slur-adjacent vocabulary the first purge missed — PENIS/PUBIC/VITA/BETH/TONY/MIL/BROS/FRAT/WORT + a scanned set, dict 11,446, banks 4,872, 0 stored-solution errors); **generator de-rarify** (`localGenerator` word scoring rewards the mainstream [10%,60%] band, not the rarest 30% — obscurity was confused with fun); **preview-grading bridge** (`PREVIEW_GRADING_BRIDGE_PUZZLES`=12: ✓/✗ stays on for every board until 12 solves, so the tutorial doesn't hand off to ungraded MEDIUM on the first free board); **3-star acceleration ramp** (linear, no cliff — see Narrative Acceleration); **the final board's one rule** (undo refused on the finale — complicity made mechanical, see Endgame); **reveal chronology cap** (`getAnimalPhase` hard-caps at 4 pre-arrival — closes the vanguard post-revelation leak, see Awareness tiers); **late-recruit catch-up boost**; **rewarded-double cadence-gated** (≤2/local-day, silenced Phase 4+) and the **Tending shrine ad removed**; **8 distinct dwell lines** (was 3); **onboarding tail trimmed** 3 taps; **Tock's monologue walls tightened**.
...
- **Attunements**: third-tier room sink (13 rooms × 3 levels, 150/200/250, phase 2+, tier-1 prerequisite, ~7,800 total) under the same `wordshift_room_upgrades` key; ShopScreen rows; restores amber scarcity through the surplus (`TENDING_COST_CAP` also 5000→1800 for an endgame middle band).
...
- **Audio**: expo-audio playing a bundled 33-file WAV pack (`assets/sounds/`: richly synthesized SFX incl. a 4-step valid-move combo ladder + sinking dark mirror, dark variants for every frequent sound, and 3 seamless ambient music beds behind a `musicEnabled` toggle; settings-gated, plays in iOS silent mode). Migrated off the deprecated expo-av, whose `VideoViewModule` failed to load on SDK 56 (`NoClassDefFoundError`) and crashed the app at launch
...
- **Sound**: a 33-file audio pack (warm additive-synth chimes with a rising combo ladder, a full dark mirror set from Phase 3, and bright/dusk/dark ambient music beds that corrupt with the descent)
...
│   ├── dictionary.ts            # 11,491-word dictionary (3-7 letters, profanity-filtered)
...
│       ├── masteryRecords.ts    # Private skill records (NG-style chase): per-difficulty solve-time trend (getSolveTrend, robust median) + best Speed-Shift round; cloud-synced key wordshift_mastery, cleared by Reset All
...
├── src/__tests__/               # ~85 suites (counts drift as features land)
...
When a letter is selected, ghost previews show what word would form at each slot position. **Validity GRADING is difficulty-gated (v1.3.0 verb redesign)**: `previewValidityVisible` in `usePuzzleGame.ts` is true ONLY on EASY (any variant except blind) and in the double_shift variant at any difficulty (its intermediate non-word state needs the guidance). When visible: valid words green with a "✓ " prefix, invalid red/dimmed with "✗ " (never color alone); a slot shows ✓ only when **both** resulting words are valid (the preview ANDs source-word validity). When hidden (MEDIUM+ standard/reverse/speed, the daily, shared-challenge boards): the ghost word renders in one neutral dimmed ink with no ✓/✗ and no valid/invalid styling — the player judges the word, which is where the skill now lives (pre-redesign, 78-83% of steps had exactly one legal move and the UI graded it for free). `isValid` is still computed internally (look-ahead/blind judgment/snapping need it); the flag controls PRESENTATION only. Screen readers announce the formed word always, the verdict only when grading is shown (no a11y leak). The drag valid-slot snapping tell follows the same rule; hidden-validity boards snap to the geometrically nearest slot. **Preview-grading bridge**: validity is ALSO shown on every board (any difficulty) until `PREVIEW_GRADING_BRIDGE_PUZZLES` (12) total puzzles are solved, so the tutorial's ✓/✗ teaching doesn't hand off to ungraded MEDIUM on the player's very first free board; the graduation toast (`getPreviewGraduationMessage`, key `wordshift_preview_graduation_seen`, device-local) marks the first genuinely ungraded board once the bridge ends. Previews are suppressed ENTIRELY only in **Blind Offering**.
...
- **Animal Whispers**: 390 lines (5 per animal per phase, 13 animals × 6 phases). Prefers animals matching puzzle trigger words.
...
- **Loop:** at global phase 5 (post-revelation), a ✴ button in the Offering Pit header opens the Tending modal. The player spends amber to "deepen the pattern," advancing a **Tending Level** on an escalating cost curve (`getTendingCost`, `TENDING_*` in `gameBalance.ts`, capped 1,800 for a playable middle band) with a once-per-local-day discount (`getNextTendingInfo`, via `dateUtils`). The service does **not** spend — the pit calls `spendAmber(cost, 'tending')` then `applyTend(cost)` (mirrors `sacrifice`/`roomUpgrades`). Milestones (5/10/25/50/100) fire a serene ceremony.
...
- **SFX pack**: 33 WAVs in `assets/sounds/` (all SFX + combo ladders + full dark set + 3 music beds), generated by `scripts/tools/generateSounds.mjs` (additive synthesis: detuned partials, 6 instrument voices, noise transients, Schroeder reverb)
...
- **Fallback prices:** the Patron / Remove-Ads CTAs render `PATRON_FALLBACK_PRICE` ($4.99) / `REMOVE_ADS_FALLBACK_PRICE` ($2.99) when live billing hasn't supplied a `priceString` (Expo Go / failed fetch), so the buttons never read as broken blanks.
