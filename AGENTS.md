...
WordShift is a React Native (Expo SDK 57) mobile word puzzle game. The app codebase is in the repo's `mobile/` directory. No Docker dependency. The core game is client-side (all gameplay state in AsyncStorage); backend features (Supabase cloud save / daily leaderboard / anonymous analytics, Sentry crash reporting) and monetization (RevenueCat in-app purchases, AdMob ads) activate when their keys are set in `app.json` → `expo.extra`, and degrade to no-ops when absent. The Supabase/Sentry keys and the **Android** monetization keys are currently configured (iOS monetization keys are still empty → NoOp on iOS); in Expo Go the native IAP/ads modules are absent, so those degrade to no-ops there regardless — Expo Go cannot validate those native integrations. Key presence also does not prove hosted backend migrations or store-side product setup.

### Running the app

- **Release target is the signed Android app installed through the Play internal-testing track.** Expo Go is a limited development preview. In this VM there is no device/emulator, so the interactive dev/test path is **web**: `cd mobile && npx expo start --web --port 8081`. The web deps (`react-dom`, `react-native-web`) are regular dependencies in `mobile/package.json` — a plain `npm install` covers them.
...
- **How the PRODUCT OWNER actually tests (read this before debugging any monetization/ad/IAP report).** The agent's dev path in this VM is web/Expo Go, but the owner does **not** use Expo Go. They test on a **physical Android device by installing the real signed `.aab` from the Play Store internal-testing track** (the same artifact `eas build` + `eas submit` ship). Implications when the owner reports behavior:
  ...
  - **Ads serve Google TEST units, not live ones.** `app.json` → `expo.extra.adsUseTestIds` is deliberately `true` through internal/closed testing (`__DEV__ || adsUseTestIds` forces test units), so a rewarded/interstitial/banner that shows is a *test* ad. That is correct — tapping your own LIVE ads on a test build is an AdMob policy violation. `app.config.js` derives the shipped value from `WORDSHIFT_RELEASE_CHANNEL`: a `production` build resolves it to `false` (live units) automatically and every other channel keeps the literal. Never hand-flip the literal; a literal `false` would put live ads into every testing channel.
...
- Start with `docs/CURRENT_BUILD.md` for current source version, merged behavior and CI evidence; use `docs/BUILD_AND_UPLOAD.md` for EAS commands. Dated audits describe their recorded snapshots.
- EAS upload rules live at repository root in `.easignore`. Preserve dependency, credential and generated-output exclusions when changing them. Keep committed runtime assets, fonts, puzzle banks and config plugins available to native builds.
- Release R8 minification/resource shrinking and the local `withAndroidOptimization` plugin are enabled. Changes to these native settings require a new signed build; web/Jest checks do not establish Android release compatibility.
...
- The suite is expected green (see the commit-specific counts in `docs/CURRENT_BUILD.md`; counts drift as features land — not load-bearing). Date-sensitive tests construct dates with local components (`new Date(2026, 1, 9)`) — never ISO strings, which parse as UTC and break in timezones behind UTC.
...
- Puzzle bank `.ts` files in `src/data/` are generated (30 files: 12 core + 3 EXPERT + 15 Lexicon). The four stored standard banks contain EASY 453, MEDIUM 338, MEDIUM_PLUS 500 and HARD 100 boards. Across all 30 families, 4,372 of 7,356 stored boards qualify; every delivered family has at least 100 boards after the September 6 gated top-up. Fresh delivery additionally applies the current vocabulary and complete-route policy; stored counts are not the eligible pool counts. Do not manually edit banks or run legacy generators or `generateBranchingTopUpA/B`, which overwrite the gated shape and lose its multi-route guarantee. The current `npm run generate:puzzles -- <standard|reverse|double> <difficulty>` command dispatches to gated drivers and writes sidecars only; prefix the difficulty with `LEX_` for Lexicon. Review the printed sidecar and use the targeted swap dry run before installation: `node scripts/swapGatedBanks.mjs <minimum-count> <family> <difficulty> --dry-run`. Remove `--dry-run` only after review, then always run `node scripts/tools/purgeProfanity.mjs` and the current vocabulary/route checks, including `node scripts/tools/auditVocabulary.mjs --branching --minimum-eligible=100` and `node scripts/tools/updateDailyBoardVersion.mjs --check`. See the Regenerating puzzle banks section in `CLAUDE.md` for supported arguments and validation.
