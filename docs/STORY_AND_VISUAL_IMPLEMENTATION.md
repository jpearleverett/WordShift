...
In the story reader and the journal transcript, only a resident or the player gets a nameplate. Narration is set in italics with no speaker: the resident's idle portrait stays in view while the room is described, but the line is never captioned "The house" (beside Ember's portrait that read as Ember speaking as the house, and the house is the one presence that must not narrate). The player's own line shows the "You" nameplate without a portrait, since the player is never drawn; the row keeps the portrait's height so the text does not jump between pages.
