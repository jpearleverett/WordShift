...
Completing or deliberately skipping a phase transition immediately opens a recruited resident's response. The completed ceremony and its owed `phase_reaction` are saved together, so a restart between them resumes the response. It takes priority over a queued house ceremony and promotional prompts. Phase 5 responds after the post-arrival ceremony, with wording that respects the saved boundary. These responses are separate from ordinary animal conversations: they do not consume unread lines or acknowledge introductions. Previously completed phases are not backfilled with reactions.
...
Every ceremony passage waits for Continue. Back returns to the previous passage; Android Back does the same after the first page and asks for skip confirmation on the first. Skip always opens a confirmation. Navigation has a 350 ms duplicate-tap guard. Authored timing controls visual effects only, and revisiting a passage does not repeat its one-shot sound cues. Reduced motion changes effects, not the time available to read.
...
In the story reader, a page's portrait belongs to that page alone (2026-09-24, owner report: portraits stuck to whoever spoke last): a resident's own line shows that resident speaking; a narration page shows the resident the line names, earliest name first, captioned in muted ink as who is on screen, or no one when it names no one; the player's line shows the "You" nameplate and no portrait. The row keeps the portrait's height so the text never jumps. Narration is never captioned "The house". The journal files each scene under its first speaker (`getStorySceneResident`).
