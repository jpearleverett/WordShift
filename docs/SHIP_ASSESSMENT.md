...
## Remediation log — addendum (readiness re-review pass)

A second readiness review (full codebase, claims independently verified) found that a few items from the first pass were marked done but were not actually wired, plus some additional polish. All of the following were implemented and verified (typecheck clean, lint 0 errors, full suite green, `npm ci` green):

| Item | Status |
|---|---|
| **Daily Challenge unreachable** | **Fixed.** The first pass changed the *unlock gate* (20→5) but the feature stayed orphaned: `DailyChallengeCard` was never rendered, `isPlayingDaily` was hardcoded `false`, and `recordDailyCompletion()` was never called. Now wired end-to-end — card in the HomeScreen header, `startDailyGame()` hook action, `isPlayingDaily` threaded through autosave/victory/modal, completion + streak-milestone amber/toast, `daily_completed` analytics. |
| **Free streak freeze never granted** | **Fixed.** `checkFreeStreakFreeze()` was defined/tested but never called; now runs once per session from App's launch effect (silent during onboarding, one-time notice otherwise). |
| **ErrorBoundary not on the crash pipeline** | **Fixed.** `componentDidCatch` now forwards to `reportError()` (was console-only), so render errors join async errors in the event log. |
| **Double-shift drop-1 mis-feedback** | **Fixed.** The first drop returned `null`, colliding with the "invalid drop" error path; it now returns a result with no `formedWord`, giving positive drop feedback (catch bounce + haptic). |
| **Speed Shift flat / too generous** | **Fixed.** Added an escalation ladder — each consecutive win shortens the next clock (5s step, 30s floor) with a "Round N" indicator; resets on time-up / fresh run. |
| **Reverse Shift silent downgrade + flat midpoint** | **Fixed.** Downgrade-to-standard now surfaces a phase-aware notice; the descent→ascent midpoint fires a celebratory haptic. |
| **FTUE undo/hint + return cadence** | **Fixed.** Tutorial now teaches undo/hint proactively and adds a "come back each day" beat to the closing onboarding lines. |
| **CI `npm ci` broken** | **Fixed.** `package-lock.json` was missing the entire `eslint` dependency tree (`package.json` requires `eslint@^9.25.0`), so the workflow's `npm ci` step would fail. Lockfile repaired (additive; runtime dep versions unchanged). |

**Corrections to the first-pass audit (verified against code, not stale docs):** the "reverse-hint null-deref" risk is a non-issue (the hint path already guards `activeSolution` with truthiness + optional chaining); stuck-detection (`hasAnyValidMove`) **is** wired into live play; pit auto-collect through puzzle 8 **is** implemented (App overrides the persistence flag); the `sky_afternoon`/`pitt_afternoon` assets are **in use** (phase ≥ 1), so only `tree.png`/`ground.png` (≈5.6KB, regenerated by tooling) are genuinely unused — not worth churning.

**Still open (needs your input / external services):** set `TELEMETRY_ENDPOINT` to a real collector (and update the privacy policy); add a crash backend (Sentry/Crashlytics) DSN — the `reportError` chokepoint is ready; fill in `eas.json` `submit.production` store credentials; monetization remains 100% plan-only (see `MONETIZATION_PLAN.md`).

---

## Remediation log — addendum (third readiness pass, AARRR + Hooked framework)

A third full-codebase review (scored against the AARRR funnel + Nir Eyal's Hooked model; all claims re-verified against code, not docs) found **no remaining hard ship blockers in the game itself** — the prior passes had closed them. Verdict: ship-ready pending production observability + a low-end-device QA pass. Verification baseline this pass: **typecheck 0 errors, lint 0 errors, full suite 37/37 suites · 1047 tests green** (run locally after `npm ci`). The following were implemented and verified:

| Item | Status |
|---|---|
| **Daily puzzle generated on-tap (stall risk)** | **Fixed.** `generateDailyPuzzle()` now memoizes per local day and a new `prewarmDailyPuzzle()` runs at launch once the daily is unlocked (App effect), so the seeded 6-letter/5-row HARD puzzle is ready instantly instead of generating when the card is tapped. Cache cleared in `clearDailyProgress()`. New tests cover memoization + prewarm + cache-clear. |
| **Daily Challenge unlock not celebrated** | **Fixed.** `getDailyChallengeIntroLines()` / `hasSeenDailyChallengeIntro` / `markDailyChallengeIntroSeen` existed but were never wired. A one-time Fox intro now fires from HomeScreen when the daily card first unlocks (same mechanism as the challenge-mode intro), marked seen on complete or early-close. |
| **Observability not enable-able without a code change** | **Fixed.** `telemetry.ts` now reads the collector URL from `app.json` `extra.telemetryEndpoint` (lazy `expo-constants` require, stays disabled by default — Node tests green). Because `reportError` already funnels async/render/global errors into the event log, configuring this one value yields **both** analytics **and** remote crash visibility with no SDK. Documented the `extra` knob in `app.json` and the live path in `errorReporting.ts`. |
| **Legal URLs 404 (GitHub Pages)** | **Half-fixed (rest needs repo settings).** `docs/privacy-policy.md` & `terms.md` now carry Jekyll `permalink` front matter, added `docs/_config.yml`, and `links.ts` URLs trailing-slash-aligned so they resolve exactly once Pages is enabled. **You must still enable GitHub Pages** (Settings → Pages → deploy from branch, `/docs`) — it is currently off (all paths 404). |
| **Project rule: "never run `npm install`"** | **Updated.** `CLAUDE.md` / `AGENTS.md` now state `npm install`/`npm ci` is allowed (fresh containers start without `node_modules`). |

**Verified-already-done (no change needed — corrections to the third-pass agent findings):** undo/hint already fire haptics + sounds (`handleUndo`/`handleHintPress`, App.tsx — the agent misread the JSX binding line); the always-enabled **RESTART** button already gives a clean, non-penalizing exit from any challenge-mode dead-end, so no "forfeit" button is needed; the "amber drought (puzzles 16–40) blocker" is overstated — early unlocks are `minPuzzles`-gated so spend can't outrun earn, and first-completion + milestone bonuses (~90 amber by puzzle 25) cover the curve; weekly/daily quests **are** surfaced (Journal Hub + claimable-amber badge on the header journal button); the "come back tomorrow" beat already exists in the onboarding close.

**Still open (needs your input / external services, unchanged):** supply a telemetry collector URL via `extra.telemetryEndpoint` (and update the privacy policy) and/or a Sentry/Crashlytics DSN; **enable GitHub Pages** so the legal URLs resolve; on-device QA pass on low-end Android (cold-start, sustained FPS, 27 MB asset decode); fill `eas.json` `submit.production` store credentials; monetization remains plan-only (`MONETIZATION_PLAN.md`) — a separate post-launch sprint; finish/commission real audio (in progress) and migrate off the deprecated `expo-av` to `expo-audio`.
