...
| B2 untested minified build | **Still the gate.** Nothing here replaces a signed, minified AAB on two physical phones through the internal track. The smoke matrix below is unchanged; add the manifest permission check (`bundletool dump manifest --bundle=<file>.aab | grep uses-permission`), a TalkBack pass over a board and a three-button-navigation pass over the sheets. Run the ad matrix on an `internal-testing`-profile build (its channel keeps Google test units); the `production`-profile candidate serves live units, so on that build only confirm ad availability and never tap an ad. |
...
| `store-policy-legal-8` (re-verified) | The SDK 57 bare template manifest declares `SYSTEM_ALERT_WINDOW` and the legacy external-storage permissions, so "Draw over other apps" appears on the Play permission list. | Add `android.permission.SYSTEM_ALERT_WINDOW`, `READ_EXTERNAL_STORAGE` and `WRITE_EXTERNAL_STORAGE` to `android.blockedPermissions`; keep `VIBRATE`. Confirm with `bundletool dump manifest --bundle=<file>.aab | grep uses-permission` (`aapt2 dump permissions` reads APKs only, so run it on a universal APK built from the bundle if you prefer that tool). |
...
- **Data safety form**: DECLARED by the owner on 2026-09-15 (previously last submitted 2026-07-02). Add "Device or other IDs" for crash reporting (Sentry) and purchase management (RevenueCat), confirm purchase history covers the subscription, and keep Advertising ID and approximate location for AdMob. Set Ads = Yes.
- **Content rating (IARC)**: DECLARED by the owner on 2026-09-15. The guidance it was answered against: fear/horror themes yes, violence as text references only, occult references yes, no user interaction, IAP and ads present. The expected outcome is Teen / PEGI 12, which matches the 13+ claim.
- **Target audience**: DECLARED by the owner on 2026-09-15. The guidance it was answered against: 13 to 15, 16 to 17 and 18+ only, and answer honestly that the app could unintentionally appeal to children (it is cute on purpose). Do not let one tester's "great for kids" pull the listing toward Families, whose ad-SDK and content rules this game cannot meet.
- **Listing website field**: DECLARED by the owner on 2026-09-15, so AdMob can verify `app-ads.txt` at `jpearleverett.github.io`; publish the EEA and US-state UMP messages in AdMob.
- **Screenshots**: the owner reviewed the launch package and uploaded the eight phone images on 2026-09-15, replacing the stale July set. Note what that does and does not settle: the September pack was rendered from the WEB build before the September 13 UI changes, so if those uploads came from that pack rather than from captures off the signed build, compare them against the signed candidate before promotion.
...
- **Supabase**: RESOLVED 2026-09-15 for the rate-limit half. `rate_limits_v1.sql` (per-install budgets on `ingest_events_v2`, `bump_words_offered` and `submit_daily_score_v2`, plus the daily plausibility floor and activity requirement) closes `backend-ops-1` and `-2`; the owner applied it with `apply_upgrade.sql`, created the retention cron, and reported the post-apply probe passing. The retention cron has also completed a successful run. Still open: the plan's disk quota and backup policy, and a signed build posting a Daily rank end to end.
...
| store-policy-legal-3 | store-policy-legal | Target-audience and IARC declarations need a deliberate answer: cute store presence vs. a 13+ horror game | medium | RESOLVED 2026-09-15 (owner declared) | `mobile/docs/store-launch/listing-en-US.json:3` |
