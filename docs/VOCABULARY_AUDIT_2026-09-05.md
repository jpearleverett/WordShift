...
The full per-word snapshot and exact query are committed in [esdb-2026-09-05.json](../mobile/scripts/vocabulary/esdb-2026-09-05.json). Its [license](../mobile/docs/licenses/ESDB-Copyright.txt) is retained. Rebuild policy with `cd mobile && npm run generate:vocabulary-policy`, then run `npm run audit:vocabulary -- .audit-reports/vocabulary-bank-audit.json --branching --minimum-eligible=100` and the vocabulary/fallback/bank tests.
...
The original audit evaluated all 30 banks, containing 9,611 historical boards. The completed top-up now supplies 4,372 eligible boards from 7,356 stored records, with at least 100 eligible boards in every family. Per-family coverage comes from the audit report the command above writes (the committed 2026-09-05 copy was deleted with the rest of that review set on 2026-09-15; the audit is reproducible, so regenerate rather than cite the old file). An additional complete-path audit checks newly eligible boards under the fresh dictionary, rather than relying only on historical solvability tests. One EASY board lost its second route when WHATS was removed and is withheld from fresh selection. The standard-bank multi-route promise must be checked using the same dictionary that the player receives.
