...
is the `version` field in `app.json` (currently `1.0.0`). An OTA update only
...
- **Native change** (new native module, a config plugin, `app.json` native
  ...
  build time, monetization keys): **bump `version`** (e.g. `1.0.0` → `1.0.1`)
...
- Bypass `version` mismatches — an update for `1.0.1` will not reach a `1.0.0`
  build, by design.
