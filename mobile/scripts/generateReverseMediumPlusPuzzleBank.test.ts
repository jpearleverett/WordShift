 ...
 * A final merge step combines all phases into src/data/puzzleBankReverseMediumPlus.ts.
 *
 * MEDIUM_PLUS uses 5-letter words with 4 rows (vs HARD's 5-letter words with 5 rows),
 * so generation is significantly faster than HARD reverse (~0.3-0.5s/puzzle).
 *
 ...
 *   for p in 0 1 2 3 4; do npx jest --config scripts/jest.config.js --no-coverage --testTimeout 600000 --maxWorkers=1 -t "phase $p" scripts/generateReverseMediumPlusPuzzleBank.test.ts; done
 *
...
function getTempPath(phase: number): string {
  return path.join(TEMP_DIR, `.reverseBankMediumPlus_phase${phase}.json`);
}
