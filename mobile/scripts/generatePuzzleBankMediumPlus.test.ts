 ...
 *
 * Generates 500 MEDIUM_PLUS-difficulty standard-variant puzzles across all 5 phase tiers,
 * enriches them with metadata, and writes to src/data/puzzleBankMediumPlus.ts.
 *
 * Run: cd mobile && npx jest --config scripts/jest.config.js --no-coverage --testTimeout 900000 scripts/generatePuzzleBankMediumPlus.test.ts
 */
...
const CHECKPOINT_PATH = require('path').join(__dirname, '..', 'src', 'data', '.bank_generatePuzzleBankMediumPlus_progress.json');
...
describe('Puzzle Bank Generator — MEDIUM_PLUS Standard', () => {
  it('generates 500 MEDIUM_PLUS standard puzzles', async () => {
    ...
    // Write output file
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'puzzleBankMediumPlus.ts');

    const fileContent = `// AUTO-GENERATED by scripts/generatePuzzleBankMediumPlus.test.ts
...
export const PUZZLE_BANK_MEDIUM_PLUS: PreGeneratedPuzzle[] = [
