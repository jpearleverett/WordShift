...
# Usage: cd mobile && bash scripts/runGatedDoubleRegen.sh <EASY|MEDIUM|MEDIUM_PLUS|HARD>

set -u
BANK="${1:-}"
case "$BANK" in
  EASY|MEDIUM|MEDIUM_PLUS|HARD) ;;
  *) echo "Usage: bash scripts/runGatedDoubleRegen.sh <EASY|MEDIUM|MEDIUM_PLUS|HARD>" >&2; exit 1 ;;
esac
cd "$(dirname "$0")/.." || exit 1
...
count_accepted() {
  node -e "try { const c = JSON.parse(require('fs').readFileSync('$CHECKPOINT','utf-8')); console.log(Array.isArray(c.puzzles)?c.puzzles.length:0); } catch { console.log(0); }"
}

echo "=== gated double driver: $BANK — starting at $(count_accepted)/${TARGET} ($(date -u +%FT%TZ)) ===" | tee -a "$LOG"
plateau=0; run=0
while :; do
