...
import { createReverseTopUpCandidates } from './reverseTopUpCandidates';
 ...
 *   cd mobile && GATED_BANK=HARD NODE_OPTIONS=--max-old-space-size=650 \
...
describe(`Gated Reverse Regeneration — ${BANK_NAME}`, () => {
  it(`regenerates the REVERSE ${BANK_NAME} bank`, async () => {
    ...
    // The corresponding common/rare checkpoint can contribute additional
    // transfer geometries, but never a copied board. Exact replay rechecks
    // every replacement under THIS bank's vocabulary, caps and rarity gates.
    const siblingKey = `${LEXICON ? '' : 'lexicon_'}reverse_${BANK_NAME.toLowerCase()}`;
    const siblingPath = path.join(__dirname, '..', 'src', 'data', `.gatedRegenReverse_${siblingKey}_progress.json`);
    let siblingSeeds: PreGeneratedPuzzle[] = [];
    if (fs.existsSync(siblingPath)) {
      const sibling = JSON.parse(fs.readFileSync(siblingPath, 'utf8'));
      try { validateGatedCheckpointPolicy(sibling); } catch { sibling.puzzles = []; }
      siblingSeeds = (sibling.puzzles as PreGeneratedPuzzle[]).filter(puzzle =>
        puzzle.wordLength === CONFIG.defaultWordLength && puzzle.words.length === (BANK_NAME === 'EASY' ? 3 : BANK_NAME === 'MEDIUM' || BANK_NAME === 'MEDIUM_PLUS' ? 4 : 5));
      for (const puzzle of siblingSeeds) seenChains.add(puzzle.words.join('-'));
    }
    // A capped rare graph benefits from reusing proved transfer geometry with
    // different words. Every mutation still passes all acceptance gates below.
    // Low-usage words and source chains are tried first to spread the supply.
    const mutationAllowed = (word: string): boolean =>
      isFairPuzzleWord(word, LEXICON || BANK_NAME === 'EXPERT') &&
      (bankWordUsage.get(word) ?? 0) < WORD_USAGE_CAP;
    const replacementWords = [...COMMON_WORDS].filter(word => word.length === CONFIG.defaultWordLength &&
      mutationAllowed(word) && (getFeaturedRank(word) <= CONFIG.featuredCeiling || isDreadWord(word)))
      .sort((a, b) => (bankWordUsage.get(a) ?? 0) - (bankWordUsage.get(b) ?? 0));
    const mutationSeeds = [...allPuzzles, ...siblingSeeds].sort((a, b) =>
      Math.max(...collectPuzzleWords(a).map(word => bankWordUsage.get(word) ?? 0)) -
      Math.max(...collectPuzzleWords(b).map(word => bankWordUsage.get(word) ?? 0)));
    let mutations = createReverseTopUpCandidates(mutationSeeds, replacementWords, mutationAllowed, { maxAttempts: 200_000 });
    const mutationNearMisses: PreGeneratedPuzzle[] = [];
    let mutationDepth = 1;
    let mutationAccepts = 0;
    ...
    for (let phaseIndex = 0; phaseIndex < phaseEntries.length; phaseIndex++) {
      ...
      while (allPuzzles.length < DELIVERY_TARGET && phaseCount < target && phaseAttempts < phaseBudget && Date.now() < phaseDeadline) {
        ...
        try {
          ...
          if (mutation.done && mutationDepth === 1 && mutationNearMisses.length > 0) {
            // A valid rare graph may need two row changes to reach the same
            // unchanged mean-rarity floor. Intermediate candidates are search
            // seeds only; they are never accepted, checkpointed or installed.
            mutationDepth = 2;
            mutations = createReverseTopUpCandidates(mutationNearMisses, replacementWords, mutationAllowed, { maxAttempts: 200_000 });
            mutation = mutations.next();
          }
          const puzzle = !mutation.done
            ? { ...mutation.value, qualityScore: 50 }
            : await generateLocalPuzzle(CONFIG.difficulty, { requireReverseSolvable: true, rarityLean: RARITY_LEAN, rareBandLo: RARE_BAND_LO });
          if (!puzzle.reverseSolution || puzzle.reverseSolution.length === 0) { rejectedNoReverse++; continue; }
          if (isReverseChainSolvable(puzzle.words, word => COMMON_WORDS.has(word)) !== 'solvable') { rejectedNoReverse++; continue; }
          ...
          if (!featuredBandOk(puzzle.words, puzzleWords)) {
            if (!mutation.done && mutationDepth === 1 && mutationNearMisses.length < 1500) mutationNearMisses.push(candidatePuzzle);
            rejectedFeatured++; continue;
          }
          if (forwardSShare(puzzle.solution as StepLike[] | undefined) > CONFIG.maxSShare) { rejectedSShare++; continue; }
          const preGenPuzzle = qualifyFreshBankPuzzle(candidatePuzzle, LEXICON || BANK_NAME === 'EXPERT', 'reverse', word => COMMON_WORDS.has(word.toUpperCase()));
          ...
          if (!mutation.done) mutationAccepts++;
    ...
    logProgress();
    process.stdout.write(`Exact-replay mutations accepted: ${mutationAccepts}\n`);
