#!/bin/bash
# Runs the reverse MEDIUM puzzle bank generator in batches until all phases are complete.
# Each invocation generates up to BATCH_SIZE (20) puzzles per phase to stay within
# memory limits, then the script re-runs for the same phase until target is met.
#
# Usage: cd mobile && bash scripts/runReverseMediumGenerator.sh

set -e

JEST_CMD="npx jest --config scripts/jest.config.js --no-coverage --testTimeout 600000 --maxWorkers=1"
SCRIPT="scripts/generateReverseMediumPuzzleBank.test.ts"

# Phase targets (must match generateReverseMediumPuzzleBank.test.ts)
...
count_phase() {
  local phase=$1
  local file="${DATA_DIR}/.reverseMediumBank_phase${phase}.json"
  if [ -f "$file" ]; then
...
echo "=== Reverse MEDIUM Puzzle Bank Generator ==="
...
echo ""
echo "Done! Check src/data/puzzleBankReverseMedium.ts"
