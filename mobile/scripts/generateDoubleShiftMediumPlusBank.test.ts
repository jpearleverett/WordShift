 ...
 *
 * Generates 500 MEDIUM_PLUS-difficulty double-shift puzzles (5 rows, 5-letter words)
 * across all 5 phase tiers.
 *
 * Run: cd mobile && npx jest --config scripts/jest.config.js --no-coverage --testTimeout 900000 scripts/generateDoubleShiftMediumPlusBank.test.ts
 */
...
const CHECKPOINT_PATH = require('path').join(__dirname, '..', 'src', 'data', '.bank_generateDoubleShiftMediumPlusBank_progress.json');
...
describe('Puzzle Bank Generator — MEDIUM_PLUS Double Shift', () => {
  it('generates 500 MEDIUM_PLUS double-shift puzzles (5 rows)', async () => {
    ...
    for (const [phaseStr, target] of Object.entries(PHASE_TARGETS)) {
      ...
      process.stdout.write(`\nPhase ${phase}: generating ${target} double-shift MEDIUM_PLUS puzzles...\n`);
      ...
      while (phaseCount < target && phaseAttempts < maxAttemptsPerPhase) {
        ...
        try {
          const puzzle = await generateDoubleShiftPuzzle('MEDIUM_PLUS');
    ...
    // Write output file
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'puzzleBankDoubleShiftMediumPlus.ts');

    const fileContent = `// AUTO-GENERATED by scripts/generateDoubleShiftMediumPlusBank.test.ts
...
export const PUZZLE_BANK_DOUBLE_SHIFT_MEDIUM_PLUS: PreGeneratedPuzzle[] = [
