 ...
 *
 * Generates 500 EASY-difficulty double-shift puzzles across all 5 phase tiers,
 * enriches them with metadata, and writes to src/data/puzzleBankDoubleShiftEasy.ts.
 *
 * Run: cd mobile && npx jest --config scripts/jest.config.js --no-coverage --testTimeout 900000 scripts/generateDoubleShiftEasyBank.test.ts
 */
...
const CHECKPOINT_PATH = require('path').join(__dirname, '..', 'src', 'data', '.bank_generateDoubleShiftEasyBank_progress.json');
...
describe('Puzzle Bank Generator — EASY Double Shift', () => {
  it('generates 500 EASY double-shift puzzles', async () => {
    ...
    for (const [phaseStr, target] of Object.entries(PHASE_TARGETS)) {
      ...
      process.stdout.write(`\nPhase ${phase}: generating ${target} double-shift EASY puzzles...\n`);
    ...
    // Write output file
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'puzzleBankDoubleShiftEasy.ts');

    const fileContent = `// AUTO-GENERATED by scripts/generateDoubleShiftEasyBank.test.ts
...
export const PUZZLE_BANK_DOUBLE_SHIFT_EASY: PreGeneratedPuzzle[] = [
