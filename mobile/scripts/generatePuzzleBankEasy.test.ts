 ...
 *
 * Generates 500 EASY-difficulty standard-variant puzzles across all 5 phase tiers,
 * enriches them with metadata, and writes to src/data/puzzleBankEasy.ts.
 *
 * Run: cd mobile && npx jest --config scripts/jest.config.js --no-coverage --testTimeout 900000 scripts/generatePuzzleBankEasy.test.ts
 */
...
describe('Puzzle Bank Generator — EASY Standard', () => {
  it('generates 500 EASY standard puzzles', async () => {
    ...
    // Write output file
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'puzzleBankEasy.ts');

    const fileContent = `// AUTO-GENERATED by scripts/generatePuzzleBankEasy.test.ts
...
export const PUZZLE_BANK_EASY: PreGeneratedPuzzle[] = [
