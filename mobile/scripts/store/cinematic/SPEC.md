...
- The interiors' painted light multiplier **never drops below 0.82 at dusk** (the scaffold's 0.62 is too dark for the luma gate).
- Fireflies from 20.2.
...
- **Out:** a 0.22 s fade with no movement.
...
| Element | Font and size | Placement |
|---|---|---|
| Claims | Figtree Bold 74 px | left-aligned at x = 96; first baseline y = 330; max width 822 px (wrap) |
...
| 7 | Panko's spice jars keep changing places. | Epunda Slab Italic | 21.55 | 24.72 | lower-centre | top band, centred |
...
- **Δ `src/world/house.js`:**
  ...
  - the dusk paint multiplier floor is 0.82;
...
- **Δ `src/world/tod.js`:** `dusk(t)` keys; sun azimuth and elevation path (2.3).
  - `makeSkyBackdrop({ tilesX: 1 })`: the single painting, band v 0.00-0.68 (sky to the river's near bank), with MirroredRepeat outside u 0-1 **faded into a haze colour** (so no mountain visibly repeats).
...
- **Δ `src/world/fire.js`:** `strokes: [{ pts, start, dur }]` (multi-stroke drawing, each stroke revealed by arc length), with 40 sparks. The house path, in local units centred above the mantel:
  ...
  - **No window.**
- **Δ `src/world/props.js`:** the jars get lettered pixel labels (a 96x32 canvas, Figtree Bold 22 px, NearestFilter); `makeShelf` gets a backboard that covers the painted shelf.
...
├─ Backdrop: skyBackdrop (single painting + fogged mirrored edges, afternoon/dusk mix, wipe), cloudCards[6]
...
| Nothing darker than sky_dusk; no storm or night skies | Only `sky_afternoon` and `sky_dusk` load. The dusk paint floor is 0.82. The luma gate YAVG ≥ 72 applies on every frame from 20.4 s (sky_dusk measures 75.08). No stars are added. The rainforest window is tinted. No fade to black. |
...
6. **Overlay bounds:** in 9:16, no overlay pixel with alpha > 0.01 outside x 96-918, y 200-1440. In 16:9, nothing below y 972.
...
3. **No tiled sky:** `tilesX = 3` repeats the mountain. Use one painting with haze-veiled mirrored edges, covering the frame.
