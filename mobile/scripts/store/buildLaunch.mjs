...
for (const file of ['app-name.txt', 'short-description.txt', 'full-description.txt', 'listing-en-US.json', 'README.md', 'claims-and-sources.md']) // Historical campaign copy is frozen in ROOT/copy for reproducible rollback.
  await fs.access(path.join(ROOT, 'copy', file));
