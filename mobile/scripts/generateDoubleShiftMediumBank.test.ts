 ...
 *
 * Generates 500 MEDIUM-difficulty double-shift puzzles (4 rows, 5-letter words)
 * across all 5 phase tiers. This bank is shared for MEDIUM, MEDIUM_PLUS, and HARD
 * since double-shift overrides force wordLength=5, targetRows=4 for all of these.
 *
 * Run: cd mobile && npx jest --config scripts/jest.config.js --no-coverage --testTimeout 900000 scripts/generateDoubleShiftMediumBank.test.ts
 */
...
const CHECKPOINT_PATH = require('path').join(__dirname, '..', 'src', 'data', '.bank_generateDoubleShiftMediumBank_progress.json');
...
describe('Puzzle Bank Generator — MEDIUM Double Shift', () => {
  it('generates 500 MEDIUM double-shift puzzles (4 rows)', async () => {
    ...
    for (const [phaseStr, target] of Object.entries(PHASE_TARGETS)) {
      ...
      process.stdout.write(`\nPhase ${phase}: generating ${target} double-shift MEDIUM puzzles...\n`);
    ...
    // Write output file
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'puzzleBankDoubleShiftMedium.ts');

    const fileContent = `// AUTO-GENERATED by scripts/generateDoubleShiftMediumBank.test.ts
...
export const PUZZLE_BANK_DOUBLE_SHIFT_MEDIUM: PreGeneratedPuzzle[] = [
