 ...
 * src/data/puzzleBankReverseMedium.ts.
 *
 ...
 *   See scripts/runReverseMediumGenerator.sh
 *
...
// MEDIUM reverse is fast (~<500ms each), so we can use a larger batch
const BATCH_SIZE = 20;

const TEMP_DIR = path.join(__dirname, '..', 'src', 'data');

function getTempPath(phase: number): string {
  return path.join(TEMP_DIR, `.reverseMediumBank_phase${phase}.json`);
}
...
async function generateBatch(phase: number, target: number, existing: PreGeneratedPuzzle[]): Promise<PreGeneratedPuzzle[]> {
  ...
  while (newPuzzles.length < batchTarget && attempts < maxAttempts) {
    ...
    try {
      ...
      // Validate it's actually 4-letter words with 4 rows
      if (puzzle.words.length !== 4) continue;
      if (puzzle.words.some(w => w.length !== 4)) continue;
