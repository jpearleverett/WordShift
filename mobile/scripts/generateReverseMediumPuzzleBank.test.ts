/**
 * Offline puzzle bank generator for WordShift — Reverse Variant MEDIUM mode.
 *
 * Generates MEDIUM-difficulty reverse-solvable puzzles (4-letter words, 4 rows)
 ...
 * A final merge step combines all phases into src/data/puzzleBankReverseMedium.ts.
 *
 * MEDIUM uses 4-letter words with 4 rows (3 moves), so generation is fast.
 *
 * Run single phase batch:
 *   npx jest --config scripts/jest.config.js --no-coverage --testTimeout 600000 --maxWorkers=1 -t "phase 0" scripts/generateReverseMediumPuzzleBank.test.ts
 *
 * Run all phases sequentially:
 *   for p in 0 1 2 3 4; do npx jest --config scripts/jest.config.js --no-coverage --testTimeout 600000 --maxWorkers=1 -t "phase $p" scripts/generateReverseMediumPuzzleBank.test.ts; done
 *
 * Merge all phases into final TypeScript file:
 *   npx jest --config scripts/jest.config.js --no-coverage --testTimeout 30000 --maxWorkers=1 -t "merge" scripts/generateReverseMediumPuzzleBank.test.ts
 */
...
// MEDIUM reverse generates fast (4 rows, 4-letter words), large batches OK.
const BATCH_SIZE = 40;

const TEMP_DIR = path.join(__dirname, '..', 'src', 'data');

function getTempPath(phase: number): string {
  return path.join(TEMP_DIR, `.reverseBankMedium_phase${phase}.json`);
}
...
async function generateBatch(phase: number, target: number, existing: PreGeneratedPuzzle[]): Promise<PreGeneratedPuzzle[]> {
  ...
  process.stdout.write(`\nPhase ${phase}: generating batch of ${batchTarget} MEDIUM reverse (${existing.length}/${target} existing)...\n`);
...
describe('Reverse MEDIUM Puzzle Bank Generator', () => {
  for (const [phaseStr, target] of Object.entries(PHASE_TARGETS)) {
    const phase = parseInt(phaseStr);

    it(`generates phase ${phase} reverse MEDIUM puzzles`, async () => {
  ...
  it('merges all phases into puzzleBankReverseMedium.ts', () => {
    ...
    // Write output file
    const outputPath = path.join(__dirname, '..', 'src', 'data', 'puzzleBankReverseMedium.ts');

    const fileContent = `// AUTO-GENERATED by scripts/generateReverseMediumPuzzleBank.test.ts
// Do not edit manually. Re-run the generator to update.
// Generated: ${new Date().toISOString()}
// Total puzzles: ${allPuzzles.length}
// All puzzles validated as reverse-solvable (requireReverseSolvable: true)
// Difficulty: MEDIUM (4-letter words, 4 rows)

import { PreGeneratedPuzzle } from './puzzleBankTypes';

export const PUZZLE_BANK_REVERSE_MEDIUM: PreGeneratedPuzzle[] = [
