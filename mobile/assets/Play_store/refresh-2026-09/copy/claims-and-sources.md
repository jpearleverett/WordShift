...
| Trailer | "Where did the day go?" | Tone tease only (boundary 8), over the scroll up the house while the afternoon turns to sunset (a transition between two recordings of the same scroll, brief section 6). It claims nothing and names no stage (boundary 9). |
...
| Trailer end card | "It's a lovely house." / "Isn't it?" | Tone tease (boundary 8), in the evening sky over the real sunset roof. The locked Underground Burrow card (Warren's room, still to be built) sits under the roof, below the level of the lines in both cuts, so the pairing never reads as placing anything under the house. |
...
| YouTube description | "All gameplay in this trailer is captured from WordShift." | Brief section 6 and boundary 17. Every picture is built from recorded frames of the game: crops, zooms and push-ins, speed changes, blur and dimming behind the cards, and dissolves. The still pictures are one freeze on Ember's finished line and the held frame of the house behind each card. No frame of the opening move is held. The only additions are: the caption plaques; a bottom border closing each dialogue sheet, from the same skin's own panel art; the 2 s sweep from afternoon to sunset during the scroll, a transition between two recordings of the same scroll in which every room is drawn whole; the end card's wordmark and lines over a flat sampled evening sky; and, in the 16:9 master, the game's sky art around the phone column and behind the cards. None of them adds or changes gameplay. Rechecked against the finished video on 2026-09-26. |
