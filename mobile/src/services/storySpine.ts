...
export function buildStoryScene(id: StorySceneId, context: StoryContext, state: StoryState): StoryScene {
  ...
  switch (id) {
    ...
    case 'echo': return scene('The same word', [
      ...
      ember("I could call this a coincidence, friend. I would like that word to be some help. It is not.", 'echo-05'),
    ...
    case 'witness': return scene('Who gets to know', [
      ...
      { id: 'share', label: 'Tell the household what we saw.', response: [
        ...
        ember("I will tell them what happened. I will say I do not know why. You can stop me if I start making it sound safer than it is.", 'witness-05'),
      ] },
    ...
    case 'plan': return scene('Which way the door faces', [
      ...
      ...(has('tarsier') ? [say('tarsier', "My watch keeps the house in sight all night. Nobody ever told me who I am watching for.", 'plan-04')] : [ember("All that work to keep something out. Or maybe, friend, that is only what we wanted the drawing to mean.", 'plan-05')]),
    ...
    case 'shelter': return scene('Something we can do', [
      ...
      has('wombat') ? say('wombat', "I can mark the road beyond the gate. Or fit this latch to a door we already have, and ask everyone to knock. Which shall I start with?", 'shelter-02')
      ...
      { id: 'road', label: 'Mark the road out.', response: [
        ...
        has('wombat') ? say('wombat', "I will keep these markers clear. I cannot tell you yet what happens past the trees.", 'shelter-06')
          : ember("I will keep the lamp filled. I wish that were the same as knowing where the road ends. It is a start.", 'shelter-07'),
      ] },
    ...
    case 'plum': return scene(has('axolotl') ? 'A little worried face' : "What the warmth keeps", has('axolotl') ? [
      say('axolotl', "This is PLUM! He is on his first lap of the tank, worried face and all. He usually stops swimming when I laugh.", 'plum-01'),
      narrator("Axel blows a crooked bubble. PLUM noses it, then turns back to him.", 'plum-02'),
      say('axolotl', "He is old for a fish. I keep forgetting how quickly somebody else can get old.", 'plum-03'),
      ...
      ember("Whatever is down there keeps the shape of everything, exactly. I am starting to wonder whether keeping the shape is enough.", 'plum-08'),
    ...
    case 'record': return scene('The corrected page', [
      ...(has('capybara') ? [say('capybara', "I wrote this in the ledger: \"I am afraid.\" Three words, complete and accurate.", 'record-01'), narrator("On his ledger, the ink now reads: EVERYTHING IS WELL.", 'record-02'), say('capybara', 'That is tidier. It is also not what I wrote.', 'record-03')] : [narrator("On a page beside the hearth, I AM AFRAID changes to EVERYTHING IS WELL. The wet ink shines.", 'record-04'), ember("I watched those letters move with no hand near them. Whatever moved them meant to help, friend. That is what makes this hard.", 'record-05')]),
      ...(witness === 'share' ? [ember("We will bring this to the shared table, beside the dated account. You asked us to compare what we saw. We need to keep doing that.", 'record-06')]
        : witness === 'private' ? [narrator('You unfold the dated account beside the ledger. For now, you and Ember compare the pages in private.', 'record-07'), ember("Keeping the paper is one thing. Remembering what was on it is another. I will remember with you.", 'record-08')] : []),
    ], "The house corrected an uncomfortable truth without asking the one who wrote it.", [
      { id: 'keep', label: 'Keep the original words.', response: [has('capybara') ? say('capybara', 'I have ink.', 'record-09') : ember('Then we will need another pen.', 'record-10'), narrator("The original sentence is copied onto a loose page and kept away from the ledger.", 'record-11')] },
      { id: 'correct', label: 'Leave the page as it is. Remember what happened.', response: [has('capybara') ? say('capybara', "I can leave the house's version on the page. I will not put my name to it.", 'record-12') : ember("We can leave the page alone. We do not have to believe what it says.", 'record-13'), narrator("You remember both sentences. The house kept only one.", 'record-14')] },
    ]);
    ...
    case 'council': return scene('Terms of a welcome', [
      ...
      ...(has('wombat') ? [say('wombat', "I'll hang a door you can open from either side. You try the latch yourself before I call the job finished.", 'council-07')] : []),
    ...
    case 'after':
      ...
      if (state.boundary && state.memories.council?.completed && !state.arrivedBeforeRevision) {
        return scene('An ordinary morning', [
          ...
          has('pangolin') ? say('pangolin', "Breakfast. I burned one side and saved the other. You may call it a disaster or a rescue, whichever you prefer.", 'after-02') : ember(`I made ${drink}. There is also toast, if you are willing to scrape it.`, 'after-03'),
          ...(has('wombat') ? [say('wombat', "A hinge is squeaking. Ordinary squeak, nothing stranger. I've got the right oil for this one.", 'after-04')] : []),
    ...
    case 'reply': return scene('Your answer', [
      ...
      { id: 'angry', label: 'I am still angry.', response: [ember("Yes. I will not rush you out of that anger. I did not tell you the whole truth when it mattered.", 'reply-03'), narrator("She lets your answer stand. Nothing corrects it.", 'reply-04')] },
      ...
      { id: 'uncertain', label: 'I do not know yet.', response: [ember("Then that is where we begin. Not knowing is allowed. Nobody gets to finish that sentence for you.", 'reply-07'), narrator("For once the silence is allowed to last. Nobody hurries to fill it.", 'reply-08')] },
    ...
    case 'old_mark': return scene('Something remained', [
      ...
      ember("We chose something once. It has lasted longer than our memory of choosing it.", 'old_mark-08'),
