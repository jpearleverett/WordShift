...
export function buildStoryScene(id: StorySceneId, context: StoryContext, state: StoryState): StoryScene {
  ...
  switch (id) {
    ...
    case 'plan': return scene('Which way the door faces', [
      ...(has('wombat') ? [say('wombat', "I always read this line on the old plan as a brace. Look which way it points."), narrator("Warren turns the drawing of the foundations around until the doorway on it faces you.")] : [narrator("A loose plan lies under the oldest hearthstone. Its arrows point in toward the house, not away from it.")]),
