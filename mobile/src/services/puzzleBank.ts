 ...
 * Each bank (standard HARD, reverse HARD, reverse MEDIUM_PLUS) has
 * independent played-ID tracking.
 */
function getStorageConfig(variant: PuzzleVariant, difficulty: Difficulty = 'HARD'): {
  key: string;
...
function deriveBankWordRecency(
  ...
  const idToWords = getIdToWordsMap(variant, difficulty);
 ...
 */
async function loadUsedPuzzles(variant: PuzzleVariant = 'standard', difficulty: Difficulty = 'HARD'): Promise<string[]> {
  const config = getStorageConfig(variant, difficulty);
 ...
 */
async function markPuzzlePlayed(puzzleId: string, variant: PuzzleVariant = 'standard', difficulty: Difficulty = 'HARD'): Promise<void> {
  const config = getStorageConfig(variant, difficulty);
  const used = await loadUsedPuzzles(variant, difficulty);
...
export async function selectPreGeneratedPuzzle(
  ...
  const storageConfig = getStorageConfig(variant, difficulty);
  const used = await loadUsedPuzzles(variant, difficulty);
  ...
  const bankWordRecency = deriveBankWordRecency(used, variant, difficulty);
  ...
  // Mark as played
  await markPuzzlePlayed(selected.puzzle.id, variant, difficulty);
