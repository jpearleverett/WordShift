...
/**
 * The story card is the game's longest reading surface, so its text sits one
 * reading gutter inside the panel's clearance token. At the bare token the
 * body, page counter and answer trays began on the last vignette ring, a few
 * dp from the inlay line, and read as touching the frame.
 */
export const STORY_READING_GUTTER_DP = 8;
...
export function getStorySceneLayout(width: number, height: number, top: number, bottom: number, fontScale = 1) {
  ...
  const contentPadX = SURFACE.panelPadX + STORY_READING_GUTTER_DP;
  const contentWidth = Math.max(1, cardWidth - contentPadX * 2);
  ...
  return {
    cardWidth, cardMaxHeight, scrollMaxHeight, contentPadX,
