...
import { StoryMemory, STORY_COPY, getStoryPages } from '../services/storySpine';
...
export const StorySceneModal: React.FC<StorySceneModalProps> = ({ memory, phase, onAdvance, onChoose, onClose }) => {
  ...
  const line = memory ? pages[Math.min(memory.page, pages.length - 1)] : undefined;
  const speakerName = line ? getStorySpeakerName(line.speaker) : '';
  const options = memory && !memory.choice && memory.page === memory.scene.lines.length - 1 ? memory.scene.options : undefined;
  const sprites = line && line.speaker !== 'narrator' && line.speaker !== 'player' ? CHARACTER_SPRITES[line.speaker] : undefined;
    ...
    <View style={[styles.overlay, { backgroundColor: theme.overlay, paddingTop: insets.top + 16, paddingBottom: insets.bottom + 16 }]} accessibilityViewIsModal>
      <PanelCard phase={phase} kind="panel" style={{ width: '100%', maxWidth: 560, maxHeight: height - insets.top - insets.bottom - 32 }}>
        <ScrollView ref={scroll} contentContainerStyle={styles.content} bounces={false} keyboardShouldPersistTaps="handled">
          {memory?.page === 0 && phase < 3 && <Image source={require('../../assets/story/kept-table.png')} resizeMode="cover" style={styles.sceneArt} accessible={false} />}
          ...
          <View style={styles.actions}>
            ...
            </Pressable>) : <CandyButton phase={phase} label={memory && memory.page >= pages.length - 1 ? STORY_COPY.finish : STORY_COPY.continue} disabled={saving} onPress={() => { void run(onAdvance); }} soundKind="none" />}
