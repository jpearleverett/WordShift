...
import { DialogueRevealBody } from './DialogueRevealBody';
...
import { AmberCountUpText } from './AmberCountUpText';
...
import { grantQuestBonus } from '../../services/questBonus';
...
interface HomeScreenProps {
   ...
   * Ember's greeting card. Opens the visitor invite exactly like tapping the
   * den; the invite never opens on its own.
   */
...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  // Narrow phones (< 340dp): the streak badge drops to its flame so the amber
  // balance beside it never truncates (accessibility-devices-6).
  const streakIconOnly = screenWidth < 340;
  // Bottom-anchored sheets are drawn under the system navigation bar now that
  // RN forces edge-to-edge on Modals (Android 15+): their last row keeps its
  // authored breathing room OR clears the bar, whichever is larger
  // (accessibility-devices-2; mirrors OfferingPitScreen / DifficultyMenu).
  const dialogueSheetBottomPad = Math.max(34, screenInsets.bottom + 12);
  const hubSheetBottomPad = Math.max(32, screenInsets.bottom + 12);
  const shopSheetBottomPad = Math.max(40, screenInsets.bottom + 12);
  ...
  // closed: a player who chained NEXT LEVEL through their first sessions met
  // three of them back to back on the landing after the first forced harvest.
  // The first eligible intro (in the existing declaration order) claims the
  // landing; the rest keep their flags unset and take a later landing.
  // HomeScreen unmounts on every navigation away, so a ref is per landing.
  const landingIntroSpentRef = useRef(false);
  ...
  // The header number counts itself up inside AmberCountUpText (a memoized
  // leaf), so the per-frame tick never re-renders HomeScreen or HouseWorld.
  // This ref only remembers the last settled balance for the gem pop below.
  const amberPulseFromRef = useRef<number | null>(null);
  ...
  const [doubleQuestOffer, setDoubleQuestOffer] = useState<{ questId: string; amber: number; receiptId: string } | null>(null);
  ...
  const loadAllData = useCallback(async () => {
    ...
    const [progressData] = await Promise.all([getFullProgress(), loadDialogueSessions()]);
    // Badge availability reads these synchronous mirrors. Warm them before
    // asking for animals, including on the first home visit after cold boot.
    updatePuzzleCount(progressData.puzzlesSolved);
    updateSessionPhase(progressData.currentPhase);
    const [roomsData, animalsData] = await Promise.all([
      getRoomsWithStatus(),
      getAnimalsWithStatus(),
    ]);
  ...
  const handleHomeLoadFailure = useCallback((error: unknown) => {
    reportError(error instanceof Error ? error : String(error), { source: 'home_load' });
    setHomeLoadError('Your house could not be loaded. Check your device storage, then try again.');
  }, []);
  const retryHomeLoad = useCallback(() => {
    void loadAllData().catch(handleHomeLoadFailure);
  }, [loadAllData, handleHomeLoadFailure]);
  ...
  useEffect(() => {
    if (dialogueFlow.showDialogue && !dialogueFlow.revealInProgress && !dialogueFlow.choiceOpen) {
      announceForA11y(dialogueFlow.dialogueText);
    }
  }, [dialogueFlow.showDialogue, dialogueFlow.revealInProgress, dialogueFlow.choiceOpen,
    dialogueFlow.dialogueText, dialogueFlow.revealSource]);
  ...
  useEffect(() => {
    ...
    if (refreshSignal > 0) retryHomeLoad();
  }, [refreshSignal, retryHomeLoad]);
  ...
  // Onboarding: the player asked to let Ember in from her greeting card. Open
  // the same visitor invite the den chip opens; if the free invite has not
  // resolved yet, remember the request and open it once it does.
  ...
  useEffect(() => {
    if (!pendingInviteRequestRef.current || onboardingStep !== 'home_empty') return;
    const next = unlockFlow.nextUnlock;
    if (hasHomeProgress && next && next.type === 'character' && next.cost === 0) {
      pendingInviteRequestRef.current = false;
      setShowInvitePrompt(true);
    }
  }, [inviteRequest, onboardingStep, unlockFlow.nextUnlock, hasHomeProgress, setShowInvitePrompt]);
  ...
  // modal by itself (ftue-4). The player opens it from the icon, or from the
  // spotlight's final "Open the Journal" tap.
  useEffect(() => {
    if (!hasHomeProgress || isOnboarding || showIntroDialogue || introOverrideLines || introOpening || pendingAnimalIntroCount > 0 || houseGiftBusy) return;
    if (landingIntroSpentRef.current) return;
  ...
  // Pop the gem when the balance climbs, scaling the pop to the SIZE of the
  // gain (a small win taps, a windfall bursts) instead of a fixed
  // magnitude-blind 1.2. The number itself counts up inside AmberCountUpText
  // (one Text re-rendering per frame, not the whole home screen); the gem
  // scales on the native driver here. Holds still (no pop) on the first read,
  // on a spend, and under reduced motion / low-tier devices.
  useEffect(() => {
    if (!hasHomeProgress) return;
    if (homeAmber === undefined) return;
    const to = homeAmber;
    const from = amberPulseFromRef.current;
    amberPulseFromRef.current = to;
    const reduced = getSettingsSync().reducedMotion || shouldSimplifyAnimations();

    if (from === null || reduced || to <= from) {
      amberPulse.setValue(1);
      return;
    }

    const gain = to - from;
    // The currency's excitement still cools with the house (a smaller ceiling
    // at Phase 4+).
  ...
  const handleClaimQuest = useCallback(async (questId: string) => {
    ...
    try {
      ...
      setDoubleQuestOffer(questId === 'all' ? null : { questId, amber: reward.amber, receiptId: `${progress.cycleCount ?? 0}:${questId}` });
  ...
  const handleDoubleQuestReward = useCallback(async () => {
    ...
    const { questId, amber, receiptId } = doubleQuestOffer;
    const newBalance = await saveWithPlayerRetry(() => grantQuestBonus(receiptId, amber), {
      title: 'Your quest amber is waiting',
  ...
  const canStartQuestReward = useCallback(() => !questClaimBusyRef.current, []);
  const handleQuestRewardBusy = useCallback((busy: boolean) => {
    questClaimBusyRef.current = busy;
    setQuestClaimBusy(busy);
  }, []);
  const handleQuestRewardError = useCallback((error: unknown) => {
    reportError(error instanceof Error ? error : String(error), { source: 'quest_bonus' });
    setQuestClaimError('Could not finish the reward. Please retry; a completed clip never needs to be watched again.');
  }, []);
  const handleCloseQuests = useCallback(() => {
    if (!questClaimBusyRef.current) setShowQuestModal(false);
  }, []);
  ...
  const localOverlayActiveRef = useRef(localOverlayActive);
  useLayoutEffect(() => { localOverlayActiveRef.current = localOverlayActive; }, [localOverlayActive]);
  // Play the house-completion celebration only once every home surface
  // has closed. Completion is detected in the same tick as Bamboo's
  ...
  useEffect(() => {
    ...
    if (localOverlayActive || storyOverlayActive || quietLanding) return;
    const timer = setTimeout(() => {
      if (introPresentationRef.current.busy() || introPresentationRef.current.pendingCount() > 0 || introSurfaceBusyRef.current || localOverlayActiveRef.current) return;
  ...
  const handleInspectStory = useCallback(() => setShowStoryInspection(true), []);
  ...
  const { showDialogue: showResidentDialogue, hasMoreToShow: hasMoreResidentDialogue, getNextAnimalWithNews } = dialogueFlow;
  const nextFriendWithNews = useMemo(() =>
    showResidentDialogue &&
    !hasMoreResidentDialogue &&
    !isOnboarding &&
    !introOverrideLines &&
    !showIntroDialogue
      ? getNextAnimalWithNews(animals)
      : null,
    [showResidentDialogue, hasMoreResidentDialogue, getNextAnimalWithNews,
      isOnboarding, introOverrideLines, showIntroDialogue, animals]);
  ...
  if (!progress || rooms.length === 0) {
    ...
    return (
      <View style={[styles.loadingContainer, { backgroundColor: PHASE_SKY_FILL[currentPhase] ?? PHASE_SKY_FILL[0] }]}>
        {homeLoadError && <>
          <Text accessibilityRole="alert" style={{ padding: 24 }}>{homeLoadError}</Text>
          <CandyButton label="Retry loading house" phase={currentPhase} onPress={retryHomeLoad} />
        </>}
      </View>
    );
  }
    ...
    <View style={[styles.container, { backgroundColor: phaseBgColor }]}>
          ...
          <View style={styles.headerLeftCluster}>
            ...
            >
              <View style={styles.amberInner}>
                ...
                <AmberCountUpText value={progress.amber} phase={progress.currentPhase} style={styles.amberCount} />
            ...
            {(progress.currentStreak > 1 || isStreakAtRisk) && (
              ...
              >
                ...
                {/* Below 340dp the header budget leaves the amber pill (the
                    ...
                    alone keeps the streak visible; the count still rides the
      ...
      <View style={styles.houseStage}>
        ...
        <HouseWorld
          ...
          onPitPress={!isOnboarding && onOpenPit ? handlePitPress : undefined}
        />
      ...
      {homeLoadError && (
        <View style={styles.reservedArrivalBanner}>
          <Text accessibilityRole="alert" accessibilityLiveRegion="assertive" style={styles.reservedArrivalText}>{homeLoadError}</Text>
          <CandyButton label="Retry loading house" phase={currentPhase} onPress={retryHomeLoad} />
        </View>
      )}
          ...
          >
            ...
            {dialogueFlow.selectedAnimal && (
              <ScrollView style={{ maxHeight: readingHeight - screenInsets.top - screenInsets.bottom - 24 }} contentContainerStyle={styles.dialogueRow} bounces={false}>
                ...
                <View style={[styles.dialogueTextCol, { paddingBottom: dialogueSheetBottomPad }]}>
                  ...
                  <TouchableOpacity
                    testID="resident-dialogue-bubble"
                    style={styles.dialogueBubble}
                    activeOpacity={0.85}
                    disabled={!dialogueFlow.revealInProgress}
                    onPress={dialogueFlow.completeReveal}
                    accessibilityRole="button"
                    accessibilityLabel={dialogueFlow.dialogueText}
                    accessibilityHint={dialogueFlow.revealInProgress ? 'Show full line' : undefined}
                    accessibilityActions={dialogueFlow.revealInProgress ? [{ name: 'activate', label: 'Show full line' }] : []}
                    onAccessibilityAction={dialogueFlow.completeReveal}
                  >
                    <NineSliceFrame
                      skin={pixelSkin.card}
                      cornerDp={CARD_CORNER_DP}
                    ...
                    />
                    <DialogueRevealBody
                      text={dialogueFlow.dialogueText}
                      source={dialogueFlow.revealSource}
                      revealing={dialogueFlow.revealInProgress}
                      charMs={dialogueFlow.revealCharMs}
                      onComplete={dialogueFlow.completeReveal}
                      style={[styles.dialogueText, { color: panelSt.body }]}
                    />
                  </TouchableOpacity>
                  {/* One-time pointer under the FIRST reveal (ftue-6): the
                      bubble has always been tappable to land the line at
                      once, but nothing ever said so. The hook decides when it
                      shows and retires it the moment that reveal ends. */}
                  {dialogueFlow.revealSkipHint ? (
                    <Text style={[styles.dialogueRevealSkipHint, { color: panelSt.muted }]} accessibilityLiveRegion="polite">
                      {dialogueFlow.revealSkipHint}
                    </Text>
                  ) : null}
      ...
      >
        ...
        >
          <SpringIn
            ...
            claimTouches
            style={[styles.compactHubModal, { maxHeight: '90%', width: '100%', maxWidth: 440, paddingBottom: hubSheetBottomPad }]}
          >
            <NineSliceFrame
              skin={pixelSkin.panel}
              cornerDp={PANEL_CORNER_DP}
      ...
      >
        ...
        >
          <SpringIn
            phase={progress?.currentPhase ?? 0}
            claimTouches
            style={[styles.shopModal, { paddingBottom: shopSheetBottomPad }]}
          >
            <NineSliceFrame
              skin={pixelSkin.panel}
              cornerDp={PANEL_CORNER_DP}
            ...
            {unlockFlow.purchaseError && (
              <Text accessibilityRole="alert" accessibilityLiveRegion="assertive" style={[styles.shopSubtitle, { color: panelSt.dangerText }]}>
                {unlockFlow.purchaseError}
              </Text>
            )}
            ...
            {areUpgradesAvailable(progress.currentPhase) && onOpenShop && (
              ...
              >
                <Text style={[styles.bevelBtnText, { color: pixelSkin.ink.secondary }]}>
                  {getHouseUpgradesPointerLabel(progress.currentPhase)}
                </Text>
              </BevelRowButton>
            )}
            </ScrollView>
      ...
      <Modal
        ...
        statusBarTranslucent
        animationType={getSettingsSync().reducedMotion ? 'none' : 'fade'}
        onRequestClose={handleCloseQuests}
      >
        <View
          style={[styles.modalOverlay, { backgroundColor: st.overlay }]}
        ...
        >
          <Pressable
            ...
            onPress={handleCloseQuests}
            accessibilityLabel="Close quests"
            accessibilityRole="button"
          />
          <SpringIn
            phase={progress?.currentPhase ?? 0}
            style={[styles.shopModal, styles.questModal, { paddingBottom: shopSheetBottomPad }]}
          >
            <NineSliceFrame
              skin={pixelSkin.panel}
              cornerDp={PANEL_CORNER_DP}
            ...
            {doubleQuestOffer && (
              <RewardedAdButton
                ...
                onReward={handleDoubleQuestReward}
                completeAfterUnmount
                canStart={canStartQuestReward}
                onBusyChange={handleQuestRewardBusy}
                onRewardError={handleQuestRewardError}
          ...
          >
            ...
            {unlockFlow.showRoomUnlock && (
              <>
                ...
                {unlockFlow.purchaseError && (
                  <Text accessibilityRole="alert" accessibilityLiveRegion="assertive" style={[styles.shopSubtitle, { color: panelSt.dangerText, marginTop: 8, fontWeight: '600', fontFamily: BODY_FONT_BOLD }]}>
                    {unlockFlow.purchaseError}
                  </Text>
                )}
                ...
                {/* Once the House Upgrades open (Phase 2), the locked-room
                    card is where a player holding idle amber is standing, so
                    it points at the recurring sink too (product-retention-3). */}
                {areUpgradesAvailable(progress.currentPhase) && onOpenShop && (
                  <BevelRowButton
                    phase={progress.currentPhase}
                    hostDark={dtHostDark}
                    ...
                    style={styles.largeAction}
                    onPress={() => {
                      unlockFlow.setShowRoomUnlock(null);
                      onOpenShop();
                    }}
                    accessibilityLabel="Open the Shop for house upgrades"
                  >
                    <Text style={[styles.bevelBtnText, { color: pixelSkin.ink.secondary }]}>
                      {getHouseUpgradesPointerLabel(progress.currentPhase)}
                    </Text>
                ...
                <View style={styles.journalSpotlightFooter}>
                  ...
                  <BevelRowButton
                    ...
                    onPress={journalSpotlightIndex < journalSpotlightLines.length - 1
                      ...
                      : async () => {
                          // The player's own tap opens the journal (the
                          // spotlight no longer auto-opens it underneath).
                          await markJournalIntroSeen();
                          setJournalSpotlightActive(false);
                          setShowJournalModal(true);
                        }
